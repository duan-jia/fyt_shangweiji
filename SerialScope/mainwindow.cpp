...
void MainWindow::vUpdateComInfo(void)
{
    ...
    /*comboBox配置波特率*/
//    ui->baudrateCb->addItem("4000000",int(4000000));
//    ui->baudrateCb->addItem("3500000",int(3500000));
//    ui->baudrateCb->addItem("3000000",int(3000000));
//    ui->baudrateCb->addItem("2500000",int(2500000));
    /*CP2104仅支持2M波特率*/
    ...
    /*comboBox配置奇偶校验*/
    ui->checkoutCb->addItem(QString::fromLocal8Bit("无"),QSerialPort::Parity(QSerialPort::NoParity));
    ui->checkoutCb->addItem(QString::fromLocal8Bit("偶校验"),QSerialPort::Parity(QSerialPort::EvenParity));
    ui->checkoutCb->addItem(QString::fromLocal8Bit("无奇校验"),QSerialPort::Parity(QSerialPort::OddParity));
...
void MainWindow::vInitSerialTx(void)
{
    ...
    /*协议定时发送相关*/
    connect(ui->protocolTxBt,&QPushButton::released,
            this,
            &MainWindow::vTxModeCfg);
    connect(ui->spinBox_2,&QSpinBox::editingFinished,
            this,
            &MainWindow::vTxModeCfg);
    connect(ui->realtimeTxCb,&QCheckBox::released,[=]()
    {
...
{
    //串口名称-仅在打开串口时更新选中
//    connect(ui->serialCb,&QComboBox::currentTextChanged,[=]()
//    {
//        this->vSerialCtr.vSerial.vSerialConfig->vSerialPortName =
//                ui->serialCb->currentData().toString();
//    });
    //串口波特率
    connect(ui->baudrateCb,&QComboBox::currentTextChanged,[=]()
    {
        this->vSerialCtr.vSerial.vSerialConfig->vSerialBaudRate =
                ui->baudrateCb->currentData().toInt();
    });
    //串口数据位
    connect(ui->stopCb,&QComboBox::currentTextChanged,[=]()
    {
        this->vSerialCtr.vSerial.vSerialConfig->vSerialStopBits =
                QSerialPort::StopBits(ui->stopCb->currentData().toInt());
    });
    connect(ui->dataCb,&QComboBox::currentTextChanged,[=]()
    {
        this->vSerialCtr.vSerial.vSerialConfig->vSerialDataBits =
                QSerialPort::DataBits(ui->dataCb->currentData().toInt());
    });
    //串口极性
    connect(ui->checkoutCb,&QComboBox::currentTextChanged,[=]()
    {
        this->vSerialCtr.vSerial.vSerialConfig->vSerialParrity  =
                QSerialPort::Parity(ui->checkoutCb->currentData().toInt());
    });
    //串口编码格式
...
void MainWindow::vTxSlotChanged(void)
{
    if(txModeCfg)
    {
        ...
        this->vSerialCtr.vQObjectTxCtr.vTimerStop();
        ui->realtimeTxCb->setChecked(false);
        ui->realtimeTxCb->setEnabled(false);
        //先断开所有，避免重复套用了链接
    ...
    else
    {
        ...
        ui->realtimeTxCb->setEnabled(true);
    }
}
//刷新Seasky协议模块
